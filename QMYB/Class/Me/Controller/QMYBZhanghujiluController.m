...
- (void)savelist:(id)response{
    ...
    if (response) {
        ...
        if (statusCode==0) {
            ...
            NSArray *dataArray=[ZhanghuList mj_objectArrayWithKeyValuesArray:response[@"data"][@"accountList"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZhanghuList *zhanghu=self.dataSourceArray[indexPath.row];
    cell.model=zhanghu;
    return cell;
}
...
- (void)addMJheader{
    MJHeader *mjHeader=[MJHeader headerWithRefreshingBlock:^{
        self.page=1;
        NSString *url=[NSString stringWithFormat:@"%@%@",APPHOSTURL,getAccountList];
        NSDictionary *dic=@{@"page":@(self.page),@"pageSize":@(self.pageSize),@"accountId":user.accountCode};
